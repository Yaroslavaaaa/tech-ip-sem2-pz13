...
func (c *Client) Publish(ctx context.Context, queueName string, body []byte) error {
	...
	c.logger.Debug("Message published", zap.String("queue", queueName))
	return nil
}
...
// SetPrefetch устанавливает количество сообщений, которые consumer может
// получить одновременно (контроль нагрузки)
